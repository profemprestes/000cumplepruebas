  ...
  if (done) {
    ...
    return (
      <div className="bg-night-grid relative flex min-h-screen w-full flex-col items-center justify-center px-6 py-10 text-white">
        ...
        />
        <div className="pixel-text mt-4 inline-block rounded-md bg-golden-coin px-3 py-1 text-[10px] uppercase tracking-widest text-night">
          Trivia completada
        </div>
        <h2 className="mt-3 text-center font-display text-3xl text-golden-coin sm:text-4xl">
          {perfect ? "¡PUNTAJE PERFECTO!" : "¡BIEN AHÍ, AVENTURERO!"}
        </h2>
        <div className="mt-4 flex items-center gap-2 text-lg">
          <Trophy className="h-6 w-6 text-golden-coin" />
  ...
  return (
    <div className="bg-night-grid relative flex min-h-screen w-full flex-col px-5 py-8 text-white">
      <header className="mx-auto w-full max-w-xl text-center">
        ...
        <h2 className="mt-3 font-display text-2xl text-golden-coin sm:text-3xl">
        ...
        <div className="mt-3 flex items-center justify-center gap-2">
          {QUESTIONS.map((_, i) => (
            <span
              key={i}
              className={`h-2 w-8 rounded-sm border-2 border-night ${
                i < idx
                  ? "bg-grass-green"
                  : i === idx
                    ? "bg-golden-coin"
        ...
        <div className="pixel-text mt-2 text-[10px] uppercase tracking-widest text-white/70">
