...
import { Calendar, MapPin, Clock, Shirt, ExternalLink, Shield } from 'lucide-react'
...
export function GuardianPanel({ onContinue }: { onContinue: () => void }) {
  ...
  return (
    <div className="bg-sky-blue relative flex min-h-screen w-full flex-col items-center overflow-x-hidden pt-8 pb-12 px-4 sm:px-6">
      ...
      <header className="relative z-10 w-full max-w-6xl text-center mb-8">
        ...
        <h2 className="font-display text-night mt-4 text-3xl drop-shadow-[2px_2px_0_#fff] sm:text-5xl lg:text-6xl">
      ...
      <div className="relative z-10 w-full max-w-6xl flex flex-col lg:flex-row lg:items-start lg:gap-10">
        ...
        <div className="flex-1 flex flex-col gap-6 order-1">
          <motion.div
            ...
            className="voxel-card border-night divide-night/10 w-full divide-y overflow-hidden rounded-xl border-4 bg-white shadow-[6px_6px_0_var(--color-teddy-brown)]"
          >
            {rows.map((r) => (
              <div
                key={r.label}
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <div
                    className={`text-sm font-bold sm:text-base ${r.highlight ? 'text-destructive font-black' : 'text-night'}`}
                  >
                    {r.value}
                  </div>
                </div>
        ...
        {/* LADO DERECHO: Mapa y Contacto */}
        <div className="flex-1 flex flex-col gap-6 order-2 mt-6 lg:mt-0">
          <motion.div
            initial={{ x: 20, opacity: 0 }}
            animate={{ x: 0, opacity: 1 }}
            className="voxel-card border-night w-full overflow-hidden rounded-xl border-4 bg-white p-0 shadow-[6px_6px_0_var(--color-teddy-brown)]"
          >
            <iframe
              title="Mapa KBOOM"
              src={EVENT.mapsEmbed}
              className="h-56 w-full contrast-125 grayscale-20 sm:h-80 lg:h-[320px]"
              loading="lazy"
            />
            <a
              href={EVENT.mapsUrl}
          ...
          <button
            onClick={onContinue}
            className="bg-golden-coin text-night border-night font-display w-full rounded-xl border-4 px-6 py-4 text-base tracking-widest uppercase shadow-[4px_4px_0_var(--color-night)] transition-all hover:translate-y-1 hover:shadow-[4px_2px_0_var(--color-night)] active:translate-y-2 active:shadow-none sm:text-lg"
          >
            🎁 Ver Guía SOBRE MI
          </button>
        </div>

        {/* Botones de Acción - visibles solo en mobile/tablet abajo */}
